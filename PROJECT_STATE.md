...
- **App version:** `v1.10.37` (set in two places — must be kept in sync):
  - `index.html` — `const VERSION = 'v1.10.37';` (used at runtime, e.g., update banner / GET_VERSION message)
  - `service-worker.js` — `const VERSION = 'v1.10.37';` (drives cache name `pracal-${VERSION}` and cache invalidation)
- **`meals.json` data version:** `1.10.13` (unchanged in T-013d.2; code-only consolidation).
...
**No active task** as of v1.10.37 ship. T-013d.2 done — Reports tab now surfaces the full expandable Body Recomp Insight card (same component as BPC home, single source of truth strengthened). T-013d.1's compact summary deleted as part of consolidation. **T-014/T-015 HOLD** per explicit user instruction — Phase 2 and Phase 3 remain blocked from auto-pickup.

17 tasks through operating model. **T-014/T-015** still blocked (HOLD).
