...
- **App version:** `v1.10.42` (set in two places — must be kept in sync):
  - `index.html` — `const VERSION = 'v1.10.42';` (used at runtime, e.g., update banner / GET_VERSION message)
  - `service-worker.js` — `const VERSION = 'v1.10.42';` (drives cache name `pracal-${VERSION}` and cache invalidation)
- **`meals.json` data version:** `1.10.17` (T-013g — Chester's เบอร์เกอร์กุ้ง m131 · 405 → 406 entries · 3rd deep-research workflow with CP Brand parent-company anchor).
...
**No active task** as of v1.10.42 ship. T-013g done — Chester's เบอร์เกอร์กุ้ง (m131) shipped after user correction "เบอร์เกอร์กุ้งมีใน CHESTER หาดีๆ" → 3rd deep-research workflow CONFIRMED via 3 primary sources. CP Brand parent-company 112g anchor used (CPF operates Chester's). **T-013g.1 deferred** (Fish Spicy Burger pending current-menu confirmation). **T-014/T-015 HOLD** per user instruction.

22 tasks through operating model. Total `meals.json` entries: **406** (was 405). **T-014/T-015** still blocked (HOLD). Cumulative deep-research across T-013f/f.1/g: **316 agents** · ~72 sources · ~55 confirmed claims · 20 killed.
