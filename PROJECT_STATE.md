...
- **App version:** `v1.10.38` (set in two places — must be kept in sync):
  - `index.html` — `const VERSION = 'v1.10.38';` (used at runtime, e.g., update banner / GET_VERSION message)
  - `service-worker.js` — `const VERSION = 'v1.10.38';` (drives cache name `pracal-${VERSION}` and cache invalidation)
- **`meals.json` data version:** `1.10.13` (unchanged in T-013d.3; code-only UI refinement).
...
**No active task** as of v1.10.38 ship. T-013d.3 done — BPC + Reports now have explicit date-range controls with full analysis-window labeling. **T-014/T-015 HOLD** per explicit user instruction — Phase 2 and Phase 3 remain blocked from auto-pickup.

18 tasks through operating model. **T-014/T-015** still blocked (HOLD).
