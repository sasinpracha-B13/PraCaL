...
- **App version:** `v1.10.40` (set in two places — must be kept in sync):
  - `index.html` — `const VERSION = 'v1.10.40';` (used at runtime, e.g., update banner / GET_VERSION message)
  - `service-worker.js` — `const VERSION = 'v1.10.40';` (drives cache name `pracal-${VERSION}` and cache invalidation)
- **`meals.json` data version:** `1.10.15` (T-013f — 5 Chester's Grill entries · 392 → 397 entries · first menu-add task with deep-research citations).
...
**No active task** as of v1.10.40 ship. T-013f done — 5 Chester's Grill entries shipped with deep-research citations. **T-013f.1 EXPANSION queued per user feedback** "บอกให้เพิ่มของ Chester ที่เป็นเมนูเดี่ยวๆให้หมด" — initial 5-entry scope was too narrow; awaiting user confirmation of expansion list before pickup. **T-014/T-015 HOLD** per explicit user instruction.

20 tasks through operating model. Total `meals.json` entries: **397** (was 392). **T-013f.1** expansion queued · **T-014/T-015** still blocked (HOLD).
