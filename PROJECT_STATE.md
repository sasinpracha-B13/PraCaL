...
- **App version:** `v1.10.43` (set in two places — must be kept in sync):
  - `index.html` — `const VERSION = 'v1.10.43';` (used at runtime, e.g., update banner / GET_VERSION message)
  - `service-worker.js` — `const VERSION = 'v1.10.43';` (drives cache name `pracal-${VERSION}` and cache invalidation)
- **`meals.json` data version:** `1.10.17` (unchanged in T-017; code-only chart interactivity).
...
**No active task** as of v1.10.43 ship. T-017 done — Weight + Waist charts in Reports now have tap-to-read (extends T-010 pattern). All 5 Reports charts now interactive. **T-014/T-015 HOLD** per user instruction.

23 tasks through operating model. **T-014/T-015** still blocked (HOLD).
