...
- **App version:** `v1.10.36` (set in two places — must be kept in sync):
  - `index.html` — `const VERSION = 'v1.10.36';` (used at runtime, e.g., update banner / GET_VERSION message)
  - `service-worker.js` — `const VERSION = 'v1.10.36';` (drives cache name `pracal-${VERSION}` and cache invalidation)
- **`meals.json` data version:** `1.10.13` (unchanged in T-013d.1; code-only surface addition).
...
**No active task** as of v1.10.36 ship. T-013d.1 done — Body Recomp Insight summary card now also lives in the Reports tab (single source of truth preserved · no duplicate classifier). **T-014/T-015 HOLD** per explicit user instruction — Phase 2 (ghost overlay · slider · auto-suggest · video frame · timer for Back) and Phase 3 (PIN lock · face crop · pose-match) remain blocked from auto-pickup.

16 tasks through operating model. **T-014/T-015** still blocked (HOLD).
...
- T-013d.1 (Body Recomp Insight summary in Reports tab) — `done` ✅ — compact card · reuses `computeBodyProgressInsight` · adds "ยังไม่ยืนยันว่ากล้ามเพิ่ม" caveat for Reports possible-recomp branch
