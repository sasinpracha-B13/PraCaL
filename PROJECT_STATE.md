...
- **App version:** `v1.10.35` (set in two places — must be kept in sync):
  - `index.html` — `const VERSION = 'v1.10.35';` (used at runtime, e.g., update banner / GET_VERSION message)
  - `service-worker.js` — `const VERSION = 'v1.10.35';` (drives cache name `pracal-${VERSION}` and cache invalidation)
- **`meals.json` data version:** `1.10.13` (unchanged in T-013d; code-only interpretation layer).
...
**No active task** as of v1.10.35 ship. **BPC Phase 1 MVP COMPLETE** — T-013d done, finishing the 4-sub-task split. Body Progress Center now has: schema + IndexedDB (a) · capture flow (b) · gallery picker + edit (b.1) · timeline + viewer + compare (c) · Recomp Insight Card with 6 status labels and 3 confidence tiers (d). All sub-tasks shipped with conservative tone discipline. **T-014/T-015 HOLD** per explicit user instruction "Do not start T-014/T-015 until I approve the next pickup" — mechanical pickup remains suspended for both Phase 2 (ghost overlay · slider · auto-suggest · video frame · timer for Back) and Phase 3 (PIN lock · face crop · pose-match).

15 tasks through operating model. **BPC Phase 1 MVP ✓ complete.** **T-014/T-015** = BPC Phase 2/3 placeholders (HOLD) · **T-016 Insight Engine** retired (T-013d delivered the body-data interpretation layer; if logging-side insight is needed later it would be a fresh task).
