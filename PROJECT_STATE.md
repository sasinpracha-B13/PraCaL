...
- **App version:** `v1.10.39` (set in two places — must be kept in sync):
  - `index.html` — `const VERSION = 'v1.10.39';` (used at runtime, e.g., update banner / GET_VERSION message)
  - `service-worker.js` — `const VERSION = 'v1.10.39';` (drives cache name `pracal-${VERSION}` and cache invalidation)
- **`meals.json` data version:** `1.10.14` (T-013e — 4 ก๋วยจั๊บ family entries · 388 → 392 entries).
...
**No active task** as of v1.10.39 ship. T-013e done — 4 ก๋วยจั๊บ family entries added (n37-n40) following the established menu-addition-protocol. **T-014/T-015 HOLD** per explicit user instruction — Phase 2 and Phase 3 remain blocked from auto-pickup.

19 tasks through operating model. Total `meals.json` entries: **392** (was 388). **T-014/T-015** still blocked (HOLD).
