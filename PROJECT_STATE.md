...
- **App version:** `v1.10.45` (set in two places — must be kept in sync):
  - `index.html` — `const VERSION = 'v1.10.45';` (used at runtime, e.g., update banner / GET_VERSION message)
  - `service-worker.js` — `const VERSION = 'v1.10.45';` (drives cache name `pracal-${VERSION}` and cache invalidation)
- **`meals.json` data version:** `1.10.17` (unchanged in T-019; code-only metabolic-calc fix).
...
**No active task** as of v1.10.45 ship. T-019 done — BMR/TDEE/protein now track current weight (7-day avg from weight log) + auto-incrementing age (`birthYear`). First hard-guardrail change; Mifflin formula byte-identical, only inputs current-derived. **T-014/T-015 HOLD** per user instruction.

25 tasks through operating model. **T-014/T-015** still blocked (HOLD). **Tier 3 (empirical TDEE from intake-vs-weight-trend)** noted as a future task (app already has the data to back-calculate true TDEE incl. metabolic adaptation).
