...
- **App version:** `v1.10.44` (set in two places — must be kept in sync):
  - `index.html` — `const VERSION = 'v1.10.44';` (used at runtime, e.g., update banner / GET_VERSION message)
  - `service-worker.js` — `const VERSION = 'v1.10.44';` (drives cache name `pracal-${VERSION}` and cache invalidation)
- **`meals.json` data version:** `1.10.17` (unchanged in T-018; SW-only bug fix).
...
**No active task** as of v1.10.44 ship. T-018 done — SW update detection fixed. Users on v1.10.44+ will see the update banner reliably on close-reopen AND on foreground-from-background. **T-014/T-015 HOLD** per user instruction.

24 tasks through operating model. **T-014/T-015** still blocked (HOLD).
